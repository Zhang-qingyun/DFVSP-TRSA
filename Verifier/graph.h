...
class IteratorRange {
   public:
    IteratorRange(const IteratorT& first, const IteratorT& firstInvalid)
        : __begin(first), __end(firstInvalid) {}

    using Iterator = IteratorT;  // make publicly visible

    IteratorT begin() { return __begin; }

    IteratorT end() { return __end; }

    bool empty() { return __begin == __end; }
...
class Graph {
   ...
   public:
    explicit Graph(int n) : _num_nodes(n), _num_edges(0), _adj_list(n) {}
    ...
    NeighborRange neighbors(const int u) const {
        return NeighborRange(_adj_list[u].cbegin(), _adj_list[u].cend());
    }
